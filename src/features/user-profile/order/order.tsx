...
const OrdersContent: React.FC<{}> = () => {
  ...
  const { data, mutate, error } = useSWR(`${process.env.NEXT_PUBLIC_DOMAIN_URL}/wp-json/wc/v3/orders?customer=${JSON.parse(localStorage.getItem("customer")).id}&consumer_key=${process.env.NEXT_PUBLIC_WOOCOMMERCE_CONSUMER_KEY}&consumer_secret=${process.env.NEXT_PUBLIC_WOOCOMMERCE_CONSUMER_SECRET}`, (url) => fetch(url).then((res) => res.json()));
  ...
  async function getOrderStatus(order) {
    ...
    if (status === "cancelled" || status === "deliverydenied" || status === "failed") {
              ...
              {data.length !== 0 ? (
                data.map((current: any) => (
                  <OrderCard
                    ...
                    status={isOrderCanceled ? "ακυρώθηκε" : progressData[orderStatusIndex - 1].label}
          ...
          {!isOrderCanceled ? <>
            ...
            {selection && (
              <OrderDetails
                ...
                subtotal={selection.line_items.map(product => product.total).reduce((accumulator, currentValue) => parseFloat(accumulator) + parseFloat(currentValue)) || "?"}
                discount={0}
