...
const CheckoutWithSidebar: React.FC<MyFormProps> = ({ token, deviceType }) => {
  ...
  const [selectedPaymentOptionValue, setSelectedPaymentOptionValue] = useState("")
  const [totalAmount, setTotalAmount] = useState("");
  const [orderLoading, setOrderLoading] = useState(false);
  const isCheckoutCartShow = useAppState('isCheckoutCartShow');
  const [selectedShippingLine, setSelectedShippingLine] = useState("acs_courier");
  ...
  useEffect(() => {
    ...
    const courierCharges = selectedShippingLine === "acs_courier" ? 3.50 : 0.00;
    const deliveryCharges = selectedPaymentOptionValue === "pay on delivery" ? 2.00 : 0;
  ...
  if (!loading && items?.length === 0) return <ChakraContainer maxW="container.lg" mt="20" mb="10" py="8"> 
  ...
  if (items?.length > 0) return (
    <Formik 
      ...
      onSubmit={async (values) => {
        ...
        const res = await createOrder({ ...values, paymentOptions, selectedShippingLine, selectedPaymentOptionValue }, items);
      ...
      validationSchema={Yup.object().shape({
        ...
        surname_2: showOtherAddressSection ? Yup.string().label("Surname") : null,
            ...
            <CheckoutWrapper>
              <CheckoutContainer>
                <CheckoutInformation>
                  ...
                  <InformationBox>
                    <DeliverySchedule>
                      <Schedules 
                        ...
                        onSelectedShippingLine={(selectedValue) => setSelectedShippingLine(selectedValue)}
                      />
                    </DeliverySchedule>
                  </InformationBox>
                  ...
                  >
                    <Payment orderLoading={orderLoading} setOrderLoading={setOrderLoading} totalAmount={totalAmount} getPaymentOptionValue={handlePaymentOptionValue} loading={loading} formik={props} deviceType={deviceType} increment={true} onPaymentConfirm={handlePaymentConfirm} />
                ...
                <CartWrapper>
                  <Sticky
                    enabled={(size.width >= 768 && !isCheckoutCartShow) ? true : false}
                    // enabled={false}
                    top={120}
                    innerZ={999}
                  >
                    <OrderInfo>
                      ...
                      <CalculationWrapper>
                        ...
                        {selectedShippingLine === "acs_courier" && <TextWrapper>
                          <Text>
                            <FormattedMessage
                          ...
                          <Text>{CURRENCY}3.50</Text>
                        </TextWrapper>}

                       {selectedPaymentOptionValue === "pay on delivery" && <TextWrapper>
                          <Text>
                            <FormattedMessage
                              id='payOnDelivery'
                        ...
                        <TextWrapper style={{ marginTop: 20 }}>
                          <Bold>
                            <FormattedMessage id='totalTextcheckout' defaultMessage='Σύνολο' />{' '}
                            <Small>
                              (
                              <FormattedMessage
                                id='vatTextcheckout'
