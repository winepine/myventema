...
export async function getAllWCProducts() {
  ...
  while (true) {
    ...
    const { data } = await axios.get(
      `${process.env.NEXT_PUBLIC_DOMAIN_URL}/wp-json/wc/v3/products?consumer_key=${process.env.NEXT_PUBLIC_WOOCOMMERCE_CONSUMER_KEY}&consumer_secret=${process.env.NEXT_PUBLIC_WOOCOMMERCE_CONSUMER_SECRET}&per_page=100&page=${page}`,
      {
