...
export function getVoucherNumberFromMetaData(meta_data: any[]) {
  return (
    meta_data.find(item => item.key === "wpslash_voucher_courier_tracking")
      ?.value || ""
  );
}

export function getTranslatedPaymentMethod(method_title: string) {
  if (method_title === "Direct Bank Transfer")
    return "Άμεση Τραπεζική Κατάθεση";
...
export function getShippingMethodCustom(shipping_method_title: string) {
  if (shipping_method_title === "Flat Rate") return "Courier (δικό μας μέσο)";
...
export function getOrderStatusLabel(orderStatus) {
  if (orderStatus === "processing") {
    return {
      ...
      description:
        "Λάβαμε την παραγγελία σου και βρίσκεται στο στάδιο επεξεργασίας.",
    };
  }
  if (orderStatus === "completed") {
    return {
      status: "completed",
      label: "Έφυγε",
      description: "Η παραγγελία σου είναι στο δρόμο!",
    };
  }
  if (orderStatus === "deliverycompleted") {
    return {
      ...
      label: "Έφτασε",
      description: "Η παραγγελία σου έφτασε!",
    };
  }
