...
import { Box, Text, Wrap, Heading, Divider, HStack, Stack, Container, Flex, Square, Image, WrapItem } from '@chakra-ui/react'
...
const ProductDetails: React.FunctionComponent<ProductDetailsProps> = ({
  ...
  const { isLoading, error, data: upsellsProducts } = useQuery('upsellsProducts', async () => {
      // const products = await Promise.all(product.related_ids.map(async (id) => {
      const products = await Promise.all([174, 136, 186].map(async (id) => {
    ...
    <>
      <ProductDetailsWrapper ref={parentRef} className='product-card' dir='ltr'>
        ...
        <ProductInfo dir={isRtl ? 'rtl' : 'ltr'}>
          <Text mb="2" color="GrayText">{product.categories?.[0]?.name}</Text>

          <ProductTitlePriceWrapper>
            {/* <ProductTitle>{product.title}</ProductTitle>  // OLD */}
            <ProductTitle>{product.name}</ProductTitle> 
            <Text color="GrayText" fontSize="20" mx="3" alignSelf="center">
              {product.attributes.find(attribute => attribute.name === "συσκευασία")?.options[0] || ""}
            </Text>
          </ProductTitlePriceWrapper>

          {/* Short Description */}
          {product.short_description && <Text color="GrayText" noOfLines={3} dangerouslySetInnerHTML={{ __html: product.short_description }} />}
          ...
          <Stack py="6">
            <HStack>
              ...
              <Text color="GrayText">{product.attributes.find(att => att.name === "διαθεσιμότητα")?.options?.[0]}</Text>
            </HStack>
            <HStack>
          ...
          {upsellsProducts.length > 0 && <>
            <Heading as="h1" fontSize="22" fontWeight="semibold" pb="6">Συνδυάζεται με</Heading>
            <Stack>
          ...
          {product.description && <Stack py="6">
            <Heading as="h1" fontSize="22" fontWeight="semibold" pb="2">Ιδιότητες</Heading>
            <p color="GrayText" dangerouslySetInnerHTML={{ __html: productDescription }} />
          </Stack>}

          {/* Features */}
            <Heading my="4" as="h1" fontSize="22" fontWeight="semibold" pb="2">Σύνθεση</Heading>
            <Wrap spacing="0px" flex="2" lineHeight={10}>
              {(product.attributes && product.attributes.length > 0) && product.attributes.map((attribute, idx) => (
                  <HStack key={idx} borderWidth="1px" w="100%" fontSize="16">
                    <Box w="35%" textAlign="right"><Text>{attribute.name}</Text></Box>
                    <Divider orientation="vertical" />
                    <Stack w="65%" spacing={0}>
                        {attribute.options && attribute.options.length > 0 && attribute.options.map((option, idx) => (
                          <Text color="GrayText" key={idx} py="0" mb="-1">{option}</Text>
                        ))}
                    </Stack>
                  </HStack>
              ))}
            ...
            {!!product.weight && <Wrap borderWidth="1px" spacing="0px" flex="2" mt={10} lineHeight={10}>
              <HStack w="100%" fontSize="16">
                  ...
                  <Divider orientation="vertical" />
                  <Box w="65%"><Text>{product.weight}</Text></Box>
              </HStack>
            </Wrap>}
      ...
      {relatedProducts.length > 0 && <RelatedItems>
