...
export const ProductCard = ({ style, data, deviceType }: Props) => {
  ...
  useEffect(() => {
    ...
    const discountInPercent = data.sale_price ? getDiscountPercent(data.regular_price, data.sale_price) : undefined;
      ...
      <Card style={style}>
        ...
        <Box>
            <Box>
              ...
              <Box px={20} py="2">
                <PriceWrapper>
                  <Price>{data.sale_price ? data.sale_price : data.regular_price} &euro;</Price>
                  {discountInPercent ? <SalePrice>{data.regular_price}</SalePrice> : null}
                </PriceWrapper>
                {/* <Title>{data.name}</Title> */}
                <Text fontSize="14" noOfLines={2} h="3.3rem" color="#0D1136">{data.name}</Text>
              </Box>
            </Box> 
        </Box>
          </a>
        ...
        <Box mt="2" px={20} pb={20}>
          <AddItemToCart deviceType={deviceType} data={data} variant="full" buttonText="Προσθήκη στο καλάθι" />
        </Box>
      </Card>
  );
};