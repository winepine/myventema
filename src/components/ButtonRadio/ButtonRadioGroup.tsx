import { Box, Container, Stack, useRadioGroup, UseRadioGroupProps, HStack, Button, IconButton, Heading, Icon, Wrap } from '@chakra-ui/react'
...
export const ButtonRadioGroup = (props: ButtonRadioGroupProps) => {
  ...
  const { isTagsLoading, tags, onAppendTag, selectedTags, onSetSelectedTags } = useTags();
  ...
  return (
    <Stack align="center" spacing="6">
      {(isProductsLoading === false || isTagsLoading === false) && (tags.length === 0 ? <Heading textAlign="center" fontStyle="italic" mb="-4">
          Αυτές είναι οι ιδανικές προτάσεις για σένα!
        </Heading> : <Heading mb="4" textAlign="center">
      ...
      {isTagsLoading || isProductsLoading ? <Container centerContent py="4">
          <Loader />
        </Container> :<>
        <Wrap
          // justify="flex-start"
        ...
        {selectedTags.length > 0 && <IconButton
            icon={<IoIosArrowBack />}
            fontSize="20"
