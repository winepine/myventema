...
const CheckboxIcon = (props: { checked: boolean }) => (
  <Box fontSize="xl" color={props.checked ? "#FD5A89" : mode('white', 'whiteAlpha.400')}>
...
export const ButtonRadio = (props: ButtonRadioProps) => {
  ...
  return (
    <ScaleFade initialScale={0.9} in={true}>
      <FormLabel overflow="hidden" backdropFilter='blur(5px)' py="0" as="label" w={{ base: "100%", md: "100%" }} {...getLabelProps()}>
        <input {...getInputProps()} aria-labelledby={id} />
        <RadioBox bg="transparent" w="max" {...getCheckboxProps()} _checked={checkedStyles} id={id}>
          <VStack  w="10rem" py="2">
            <VStack overflow="hidden" textAlign="center">
              <Box color={state.isChecked ? "#FD5A89" : "white"} aria-hidden fontSize="4xl">
                {icon}
              </Box>
              <Stack spacing="0">
                <Text color={state.isChecked ? "#FD5A89" : "white"} fontWeight="extrabold" fontSize="xl">
                  {label}
                </Text>
                <Text fontStyle="italic" color={state.isChecked ? "#FD5A89" : "white"} fontSize="sm">{`(${description})`}</Text>
              </Stack>
            </VStack>
